...
$.ajax({
  ...
  success: function (data) {
    if (data.length > 0) {
      ...
      //商品规格
      document.querySelector('#specification').innerHTML = `${data[0].specification}`
      //详情图片
      let imgsrcs = data[0].descImg.split(';');
      ...
      // 放大镜渲染
      let zoomImg = data[0].bannerImg.split(';');
      let zoomHtml = '';
      for (let i in zoomImg) {
        zoomHtml += `
        <li><img
              src="${zoomImg[i]}"/>
            </li>
      ...
      document.querySelector('#exzoomImgLi').innerHTML = zoomHtml;
    }
  },
  error: function (err) {
  }
...
$('#add_shop_cart').click(function () {
  var valuess = document.querySelector('.number').innerHTML;
  console.log(valuess)
  $.ajax({
    type: "POST",
    url: "http://127.0.0.1:4000/api/userOrder/add_user_order/cart",
    async: false, //同步
    dataType: "json",
    data: {
      userId: getCookie('userId'),
      goodId: getCookie('goodid'),
      num: valuess
    },
    success: function (data) {
